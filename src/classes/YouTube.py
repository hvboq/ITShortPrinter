...
검수 기준:
- 전체 분량이 보통 말하기 속도로 30~40초에 맞는다.
- 첫 문장이 질문형 훅으로 시작하고, 첫 3초 안에 답의 방향까지 즉시 예고한다. 질문만 던지고 답을 미루면 수정한다.
...
""".strip()
    ...
    def generate_script(self) -> str:
        ...
        if self.news_article:
            ...
            prompt = f"""
        {sentence_length}문장 이내, 보통 말하기 속도로 30~40초 분량의 한국어 유튜브 쇼츠 대본을 작성해.
        ...
        첫 문장은 짧은 질문형 훅으로 시작하고 같은 문장 안에서 답의 방향을 즉시 예고해. 이 둘이 첫 3초 안에 들려야 하며 질문만 던지고 답을 미루면 안 돼.
    ...
    def upload_video(self) -> bool:
        """
        Uploads the video to YouTube through the official YouTube Data API.
