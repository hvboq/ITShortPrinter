...
from config import get_env_var, get_image_provider, get_nanobanana2_model, get_subtitle_max_chars, get_tts_provider
...
def main() -> int:
    ...
    for idx, article in enumerate(selected, 1):
        ...
        mark_shorts_status(article, archive_status, rank=daily_rank, video_path=abs_path)
        manifest.append(info)
        MANIFEST.write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding='utf-8')
        print(
            f"GENERATE_{idx}_DONE|date={archive_date}|daily_rank={daily_rank}|path={abs_path}|"
            f"duration={info['duration']}|bytes={info['bytes']}|review_status={archive_status}",
            flush=True,
        )
